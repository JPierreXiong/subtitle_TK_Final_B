...
export const mediaTasks = pgTable(
  'media_tasks',
  {
    ...
    // New field: rewritten scripts (JSONB for flexibility)
    rewrittenScripts: jsonb('rewritten_scripts').$type<Array<{
      style: string;
      en: string;       // English master version
      target: string;   // Final localized version
      lang: string;     // Target language code
      createdAt: string;
    }>>().default([]), // Array of { style, en, target, lang, createdAt }
