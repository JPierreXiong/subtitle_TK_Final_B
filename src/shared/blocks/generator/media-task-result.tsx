  ...
  const fetchTaskData = async () => {
    ...
    try {
      ...
      if (code === 0 && data) {
        setTaskData({
          ...data,
          // Ensure rewrittenScripts is parsed correctly
          rewrittenScripts: (() => {
            try {
              if (!data.rewrittenScripts) return undefined;
              if (typeof data.rewrittenScripts === 'string') {
                return JSON.parse(data.rewrittenScripts);
              }
              if (Array.isArray(data.rewrittenScripts)) {
                return data.rewrittenScripts;
              }
              return undefined;
            } catch (e) {
              return undefined;
        ...
        <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          {loading ? (
            ...
            <div className="mt-6 space-y-6">
              ...
              {/* Rewrite Section */}
              {(taskData.status === 'extracted' || taskData.status === 'completed') && (
                <div className="space-y-4 pt-4 border-t">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold">Viral Script Rewrite</h4>
                    <RewriteTrigger
                      taskId={mediaTaskId}
                      currentStatus={taskData.status}
                      onRewriteStart={() => {
                        // Refresh task data when rewrite starts
                        fetchTaskData();
                      }}
                    />
                  </div>
                  <RewriteDisplay taskId={mediaTaskId} />
                </div>
              )}
