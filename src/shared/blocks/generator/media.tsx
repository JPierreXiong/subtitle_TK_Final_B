...
export function MediaExtractor({
  ...
  // Computed: is extracting (processing, pending, or downloading)
  const isExtracting = isPolling && (
    taskStatus?.status === 'processing' || 
    taskStatus?.status === 'pending' || 
    (taskStatus?.status as string) === 'downloading'
  );
  ...
  const getProgressText = () => {
    ...
    if ((taskStatus.status as string) === 'downloading') {
      return 'Uploading video to storage (approx. 3-5 mins)...';
    }
