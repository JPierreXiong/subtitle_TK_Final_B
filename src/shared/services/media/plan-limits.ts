...
export async function getUserPlanType(userId: string): Promise<PlanType> {
  try {
    ...
    // If plan_type column doesn't exist, return default 'free'
    // This handles cases where database migration hasn't been run yet
    console.warn(`[getUserPlanType] Error querying plan_type for user ${userId}:`, error.message);
    console.warn(`[getUserPlanType] Returning default plan type 'free'`);
    return 'free';
  }
}
...
export async function getUserPlanLimits(userId: string) {
  ...
  try {
    ...
    // If query fails (e.g., missing columns), continue with defaults
    console.warn(`[getUserPlanLimits] Error querying user record for ${userId}:`, error.message);
  }
